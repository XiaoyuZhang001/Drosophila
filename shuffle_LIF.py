...
if __name__ == '__main__':
    ...
    print('Shuffle')
    for swap in [0.005]:#0.01,0.03,0.05,0.1,0.2]:
            ...
            while (flag <= 10):
                ...
                for node in G1.nodes():
                    ...
                    for i in range(len(temp)):
                        # I_syn += G[temp[i][0]][temp[i][1]]['weight'] * G[temp[i][0]][temp[i][1]]['state']
                        I_syn += G1[temp[i][0]][temp[i][1]]['weight'] * abs(V_th - G1.nodes[temp[i][1]]['V_m']) * G1.nodes[temp[i][0]]['state']
                        if G1[temp[i][0]][temp[i][1]]['weight'] > maxweight:
                            ...
                            #for j in range(len(t1)):
                                ...
                                # print(1111)
                        #print(p)
                    I_syn = I_syn/maxweight*33#33#22#33.3
                    # G.nodes[node]['I_syn'] = G.nodes[node]['I_syn'] + I_syn
                    # I_syn = G.nodes[node]['I_syn']
                    # print(I_syn,end='|')
                    if len(temp) == 0:
                            # print(p)
                        ...
                        V_m = G1.nodes[node]['V_m']
                        dV = (dt / tau_m) * (R_m * I_syn - V_m + V_reset) # 衰减到重置电位
                        # print(dV, end='|')
                        V_m += dV
                ...
                #     if G_deepcopy.nodes[node]['state'] > 0.9:
                #         G_deepcopy.nodes[node]['state'] = 1
                json_data = nx.node_link_data(nx.create_empty_copy(G1, with_data=True))
                filename = "./results_LIF/degree/" +str(swap)+'/'+str(test_number)+'/' +str(flag) + ".json"
                with open(filename, 'w') as f:
                    json.dump(json_data, f)
