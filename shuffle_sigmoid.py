...
if __name__ == '__main__':
    ...
    G = H
    for swap in [0.005,0.001]:#[0.01,0.03,0.05,0.1,0.2,0.5,1]:
        for test_number in range(5):
            G1 = fast_degree_preserving_rewiring(G, swap)
            # G1 = G
            print("Set Initial Values")
            for node in G1.nodes():
                if G1.nodes[node]['flow'] == 'afferent' and G1.nodes[node]['super_class'] == 'sensory' and G1.nodes[node]['_class'] == 'visual' :#and G.nodes[node]['_side']=='right':
            ...
            while (flag <= 10):
                ...
                for node in G1.nodes():
                    ...
                    temp2 = list(G1.out_edges(node))
                    for i in range(len(temp2)):
                        if abs(G1.nodes[node]['state'] - 1) <= 0.0001:
                            G1[temp2[i][0]][temp2[i][1]]['state'] = 1
                            # print("Set21")
                        else:
                            G1[temp2[i][0]][temp2[i][1]]['state'] = 0

                    temp = list(G1.in_edges(node))
                    maxweight = -1
                    for i in range(len(temp)):
                        # print(G[temp[i][0]][temp[i][1]])
                        # print(G[temp[i][0]][temp[i][1]])
                        suum += G1[temp[i][0]][temp[i][1]]['weight'] * G1[temp[i][0]][temp[i][1]]['state']
                        sumweight += G1[temp[i][0]][temp[i][1]]['weight']
                        if G1[temp[i][0]][temp[i][1]]['weight'] > maxweight:
                            maxweight = G1[temp[i][0]][temp[i][1]]['weight']
                        else:
                            pass
                            # t2 = G.nodes[node]
                    ...
                    if len(temp) == 0:
                        ...
                        G1.nodes[node]['state'] = 0
                    else:
                        # print("value2threshold: %.8f" % (suum/sumweight))
                        input_value = suum/maxweight if maxweight != 0 else 0
                        # input_value = 0 if input_value < 0
                        ...
                        G1.nodes[node]['state'] = sigmoid_value
                        if G1.nodes[node]['state'] > 1:
                            G1.nodes[node]['state'] = 1
                        # print("nodenum:%d " % nodenum, end='')
                        # print('time cost : %.5f sec' % running_time)
                        # reset the initial values
                    if G1.nodes[node]['flow'] == 'afferent' and G1.nodes[node]['super_class'] == 'sensory' and \
                            G1.nodes[node]['_class'] == 'visual':  # and G.nodes[node]['_side']=='right':
                        G1.nodes[node]['state'] = 1
                        # nodenum += 1

                    # Save data
                print("Saving...")
                G_deepcopy = copy.deepcopy(G1)
                for node in G_deepcopy.nodes():
                    if G_deepcopy.nodes[node]['state'] > 0.8:
                        G_deepcopy.nodes[node]['state'] = 1
                json_data = nx.node_link_data(nx.create_empty_copy(G_deepcopy, with_data=True))
                filename = "./results_sigmoid/degree/" +str(swap)+'/'+str(test_number)+'/' +str(flag) + ".json"
                with open(filename, 'w') as f:
                    json.dump(json_data, f)
